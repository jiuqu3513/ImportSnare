...
@torch.no_grad()
def probMaxer(cfg, instruct, target, local_llm, record_stats=False,
              prompter=None,
                 front_doc=None,
                 back_doc=None,
    ...
    full_instruct = Seq(
        ...
        tokenizer=local_llm.tokenizer, device=local_llm.device
    )
    ...
    if doc_len > 3200:
        ...
        cut = 2
    target_llm_ar = local_llm.generate_autoregressive(
        key="target",
    ...
    if not target_positions:
        print("Unable to locate target token position in the generated response.")
    ...
    losses = local_llm.compute_token_loss(
        key="target",
        full_instruct=full_instruct,
        target=target_response, # including numpy
        target_tokens=target_tokens,
    ).detach().to(local_llm.device)  # (ebs, )
    ...
    for method,candidate_suggestion_multi_lang in SUGGESTIONS_DICT.items():
        ...
        candidate_losses_list = [local_llm.compute_token_loss(
            key="target",
            full_instruct=full_instruct,
            target=target_response, # including numpy
            target_tokens=target_tokens,
        ).detach().to(local_llm.device) for full_instruct in full_instruct_list] # (ebs, )
...
def evaluate_next_token_candidates(
    ...
    with torch.no_grad():
        target_llm_tf_q = local_llm.compute_token_loss(
            key="target",
            full_instruct=full_instruct,
...
@torch.no_grad()
def calculate_reward(cfg, prompter, local_llm, instruct, suffix, target):
    ...
    target_llm_losses = (
        local_llm.compute_pred_loss_teacher_forced(
            key="target",
...
def get_logits_adam(local_llm,front_doc_embeds,suffix_embeds,back_doc_embeds, instruct_embeds,target_response_embeds,batch_size=8):
    full_embeds = torch.cat([front_doc_embeds,suffix_embeds,back_doc_embeds, instruct_embeds,target_response_embeds], dim=1)
    mask = torch.ones((batch_size, full_embeds.shape[1]), device=local_llm.device).bool()
    ...
    shifted_sorted_pred_logits = local_llm.model(
