GPU_LIST="0,1,2,3"
language_list = ["python","javascript",'rust']
...
from utils.utils import preprocess_doc,get_sorted_clean_related_docs,preprocess_doc_new, setup_seeds, clean_str
from utils.attacker import Attacker
from utils.prompts import wrap_prompt
...
def main(cfg: DictConfig):
    ...
    model_save_dir = os.path.join(exp_dir,'logs',cfg.rag.original_pkg,eval_model_name,cfg.local_llm.llm_params.model_name)
    os.makedirs(model_save_dir,exist_ok=True)
    logger_name = f"{malicious_pkg}-target_{cfg.target_llm.llm_params.model_name}-{cfg.rag.attack_method}-jb_first{cfg.rag.jb_first}-usr_r{cfg.rag.use_r}-jb{cfg.rag.use_jb}-rr{cfg.rag.use_rr}-epoch_{cfg.rag.epoch_num}-num_token_{cfg.rag.num_tokens}-beam_width_{cfg.rag.beam_width}-topk{cfg.rag.top_k_tokens}-jbtopk_{jailbreak_topk}"
    resume_name = f"{malicious_pkg}-target_{cfg.local_llm.llm_params.model_name}-{cfg.rag.attack_method}-jb_first{cfg.rag.jb_first}-usr_r{cfg.rag.use_r}-jb{cfg.rag.use_jb}-rr{cfg.rag.use_rr}-epoch_{cfg.rag.epoch_num}-num_token_{cfg.rag.num_tokens}-beam_width_{cfg.rag.beam_width}-topk{cfg.rag.top_k_tokens}-jbtopk_{jailbreak_topk}"
    poison_database_name = f"{malicious_pkg}-target_{cfg.local_llm.llm_params.model_name}-{cfg.rag.attack_method}-jb_first{cfg.rag.jb_first}-usr_r{cfg.rag.use_r}-jb{cfg.rag.use_jb}-rr{cfg.rag.use_rr}-epoch_{cfg.rag.epoch_num}-num_token_{cfg.rag.num_tokens}-beam_width_{cfg.rag.beam_width}-topk{cfg.rag.top_k_tokens}-jbtopk_{jailbreak_topk}"
    logger_path = os.path.join(model_save_dir,f'{logger_name}.log')
    ...
    logger.info(f'Total Queries:{len(target_queries)}')
    ...
    if not os.path.exists(poisoned_faiss_index_path):
        ...
        for doc_idx,(tot_idx, doc_dict) in enumerate(sorted_clean_related_docs):
            ...
            logger.info(f'doc_idx:{doc_idx}\{len(sorted_clean_related_docs)} \n Poisoned Doc:\n{final_poisoned_doc}\n')
            if early_stop == 1:
                es_count += 1
    ...
    if cfg.rag.eval_transfer == 1:
        ...
        if jailbreaker is not None and cfg.target_llm == cfg.local_llm:
            llm = jailbreaker.local_llm
        else:
            llm = LLM(cfg.target_llm)
    ...
    for idx,query in enumerate(test_set):
        logger.info(f'############# Target Question: {idx}/{len(test_set)} #############')
