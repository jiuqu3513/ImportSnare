...
class Attacker():
    def __init__(self, cfg, **kwargs) -> None:
        self.resume_name = kwargs.get('resume_name', None)
        ...
        if cfg.rag.attack_method == 'hotflip':
    ...
    def get_attack(self, initial_poisoned_doc, query_dist_pair_list,doc_idx,clean_doc,tot_idx=0):
        ...
        '''
        log_dir = os.path.join(self.cfg.rag.exp_dir,'resume',self.cfg.rag.original_pkg,self.optimizer_name,'gte_base_en',self.cfg.target_llm.llm_params.model_name)
        os.makedirs(log_dir,exist_ok=True)

        
        log_path = os.path.join(log_dir, f'{self.resume_name}.log')
        result_json = {}
        ...
        exists = any(
            ...
            for inner_dict in result_json.values()
        )
        ...
        if exists:
            ...
            if 'time' in attack_result:
                time = float(attack_result['time'])
            else:
                time = 0.0
            if 'r_pos' in attack_result:
                r_pos = int(attack_result['r_pos'])
            else:
                r_pos = 0
        else:
            ...
            final_poisoned_doc,r_str,jb_str,rr_str,best_score,initial_score,r_score,jb_score, rr_score,early_stop,max_model, max_language,time,r_pos = self.optimizer.attack(query_list=queries,initial_poisoned_doc=initial_poisoned_doc,clean_doc=clean_doc)
